...
export default function tabDefaults(_ctx: Context, _opts?: Record<string, never>): Record<string, types.site_core.TabDescriptor[]> {
    const jsonDl = (suffix: string) => ({ label: "download .json", suffix });
    return {
        StructureDefinition: [
            { id: "content",     label: "Content",               kind: "main",      suffix: "",                   render: "renderCanonical" },
