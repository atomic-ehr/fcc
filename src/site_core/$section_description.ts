...
export default function $section_description(ctx: Context, opts: { resource: types.fcc.Resource }): { title: string; id: string; html: string } | null {
    ...
    const { intro } = ctx.fns.site_core.notesFor(ctx, { resource: r });
    const desc = d.description as string | undefined;
    const descHtml = desc ? `<div class="prose prose-slate prose-sm mt-2 max-w-3xl">${ctx.fns.site_md.mdToHtml(ctx, { md: desc })}</div>` : "";
    const html = `${descHtml}${ctx.fns.site_core.introBlock(ctx, { html: intro })}`;
