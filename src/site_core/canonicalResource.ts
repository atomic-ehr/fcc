...
export default async function canonicalResource(
    ...
    const resolved = ctx.fns.site_core.tabsFor(ctx, { resource: r });
    ...
    const strip = ctx.fns.site_core.canonicalTabStrip(ctx, { resource: r, activeId: active?.d.id ?? wanted });
    const renderKey = active?.d.render ?? "renderCanonical";
    const fn = ctx.fns.site_core.resolveFn(ctx, { key: renderKey }) ?? ctx.fns.site_core.renderCanonical;
