...
export default function tabsFor(
...
): Array<{ d: types.site_core.TabDescriptor; href: string; rawName?: string }> {
    const r = opts.resource;
    const reg = (ctx.state.site?.tabRegistry ?? {}) as Record<string, types.site_core.TabDescriptor[]>;
    ...
    const out: Array<{ d: types.site_core.TabDescriptor; href: string; rawName?: string }> = [];
    for (const d of set) {
        if (d.avail) {
            const pred = ctx.fns.site_core.resolveFn(ctx, { key: d.avail });
