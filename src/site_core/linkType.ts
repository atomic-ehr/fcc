...
export default function linkType(ctx: Context, opts: { code: string; profile?: string }): string {
    const pill = ctx.fns.site_core.pillType(ctx, { t: opts.code });
    ...
    if (opts.profile) return ctx.fns.site_core.linkCanonical(ctx, { url: opts.profile, short: true });
    ...
    if (!href) return pill;
    return `<a class="hover:underline" href="${ctx.fns.site_core.htmlEscape(ctx, { s: href })}" target="_blank" rel="noopener">${pill}</a>`;
}
