...
export default function sdListTable(ctx: Context, opts: { kind: "profile" | "extension" }): string {
    ...
    out.sort((a, b) => ctx.fns.site_core.titleOf(ctx, { resource: a }).localeCompare(ctx.fns.site_core.titleOf(ctx, { resource: b })));
    ...
    const rows = out.map(r => {
        ...
        return `<tr class="border-t border-slate-100 align-top">
            ...
            <td class="px-3 py-1.5 text-sm text-slate-600">${ctx.fns.site_md.mdInline(ctx, { md: desc })}</td>
        </tr>`;
    }).join("");
