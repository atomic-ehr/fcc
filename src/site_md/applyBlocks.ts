...
export default function applyBlocks(ctx: Context, opts: { html: string }): string {
    ...
    const reg = (ctx.state.site?.blockRegistry ?? {}) as Record<string, types.site_md.BlockDescriptor>;
    ...
    h = h.replace(
        ...
        (_m, tag, attrs, inner, raw) => {
            const ial = ctx.fns.site_md.parseIal(ctx, { raw });
            ...
            const d = reg[cls]!;
            if (d.render) {
                const fn = ctx.fns.site_core.resolveFn(ctx, { key: d.render });
