...
export default function $section_formalViews(ctx: Context, opts: { resource: types.fcc.Resource }): { title: string; id: string; html: string } | null {
    ...
    const h = ctx.fns.site_profile.sdHrefs(ctx, { resource: r });
    const elements = ((d.differential as { element?: Array<Record<string, unknown>> } | undefined)?.element) ?? [];
    const snapEls  = ((d.snapshot     as { element?: Array<Record<string, unknown>> } | undefined)?.element) ?? [];
    ...
    const diffPanel  = ctx.fns.site_profile.elementTable(ctx, { elements, defnHref: h.definitions });
    const snapPanel  = snapEls.length ? ctx.fns.site_profile.elementTable(ctx, { elements: snapEls, defnHref: h.definitions }) : "";
    const keyPanel   = keyEls.length ? ctx.fns.site_profile.elementTable(ctx, { elements: keyEls, defnHref: h.definitions }) : "";
    const bindPanel  = ctx.fns.site_profile.bindingsTable(ctx, { elements });
    const constPanel = ctx.fns.site_profile.constraintsTable(ctx, { elements });

    const ANCHOR: Record<string, string> = { key: "tabs-key", differential: "tabs-diff", snapshot: "tabs-snap", bindings: "tabs-bind", constraints: "tabs-inv" };
    ...
    if (snapPanel)  panels.push({ key: "snapshot", label: "Snapshot", html: snapPanel });
    ...
    const innerTabs = ctx.fns.site_profile.profileTabs(ctx, { tabs: panels.map(p => ({ key: p.key, label: p.label, anchor: ANCHOR[p.key] })) });
    const innerHtml = panels.map(p =>
        `<div id="${ANCHOR[p.key]}" data-show="$sdtab === '${p.key}'"${p.key === activeInner ? "" : ` style="display:none"`}>${p.html}</div>`,
    ).join("");

    const baseLink = ctx.fns.site_core.linkCanonical(ctx, { url: d.baseDefinition as string });
    const html = `
        <p class="mt-1 text-xs text-slate-500">This structure is derived from ${baseLink}.</p>
        <div data-signals="{sdtab: '${activeInner}'}">
            ${innerTabs}
            ${innerHtml}
            ${ctx.fns.site_profile.flagLegend(ctx)}
        </div>
        <script>${ctx.fns.site_profile.tabHashScript(ctx)}</script>`;
