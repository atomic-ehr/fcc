...
export default function renderDefinitionsPage(ctx: Context, opts: { resource: types.fcc.Resource; strip?: string }): string {
    ...
    const body = `
        ...
        ${opts.strip ?? ctx.fns.site_core.canonicalTabStrip(ctx, { resource: r, activeId: "definitions" })}
        ${ctx.fns.site_core.urlVersionStrip(ctx, { d })}
        <h2 class="mt-6 text-lg font-semibold text-slate-900">Detailed Descriptions</h2>
        <div class="mt-2">${ctx.fns.site_profile.detailTable(ctx, { elements, anchors: true })}</div>
    `;
