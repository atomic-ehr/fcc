...
export default function elementRow(ctx: Context, opts: { e: Record<string, unknown>; isLast?: boolean; defnHref?: string }): string {
    ...
    const indent = ctx.fns.site_profile.treeIndent(ctx, { path, isLast: opts.isLast });
    ...
    const flags = ctx.fns.site_profile.flagsCell(ctx, { e });
    ...
    if (!typeHtml && !path.includes(".") && path) typeHtml = ctx.fns.site_core.linkType(ctx, { code: path });

    const binding = e.binding as { strength?: string; valueSet?: string } | undefined;
    const bindingHtml = binding
        ? `${ctx.fns.site_core.tagBindingStrength(ctx, { s: binding.strength ?? "" })} ${ctx.fns.site_core.linkCanonical(ctx, { url: binding.valueSet, short: true })}`
        : "";
    ...
    return `
        <tr class="hover:bg-slate-50/60 even:bg-slate-50/40">
            ...
            <td class="px-3 py-1 align-top text-xs text-slate-600">
                ${ctx.fns.site_md.mdInline(ctx, { md: desc })}
