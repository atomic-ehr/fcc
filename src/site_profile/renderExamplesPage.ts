...
export default function renderExamplesPage(ctx: Context, opts: { resource: types.fcc.Resource; strip?: string }): string {
    ...
    const examples = ctx.fns.site_profile.examplesForProfile(ctx, { profile: r });

    const list = examples.length
        ? `<ul class="mt-2 list-disc space-y-0.5 pl-6 text-sm">
            ${examples.map(ex => `<li><a class="text-sky-700 hover:underline" href="${ctx.fns.site_core.pageHref(ctx, { resource: ex })}">${esc(ctx.fns.site_core.titleOf(ctx, { resource: ex }))}</a></li>`).join("")}
        </ul>`
    ...
    const body = `
        ...
        ${opts.strip ?? ctx.fns.site_core.canonicalTabStrip(ctx, { resource: r, activeId: "examples" })}
