...
export default function $section_quickStart(ctx: Context, opts: { resource: types.fcc.Resource }): { title: string; id: string; html: string } | null {
    ...
    const table = ctx.fns.site_profile.quickStartTable(ctx, { resourceType: rt });
    if (!table) return null;
    const html = `<p class="mt-1 text-xs text-slate-500">Search parameters defined for the <code>${ctx.fns.site_core.htmlEscape(ctx, { s: rt })}</code> resource in this IG.</p>
