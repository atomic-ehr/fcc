...
export default function detailTable(ctx: Context, opts: { elements: Array<Record<string, unknown>>; anchors?: boolean }): string {
    ...
    const blocks = opts.elements.map(e => {
        ...
        const card = ctx.fns.core.formatCard(ctx, { min: e.min, max: e.max });

        const types = (e.type as Array<{ code: string; profile?: string[] }> | undefined) ?? [];
        const typeHtml = types.map(t =>
            ctx.fns.core.linkType(ctx, { code: t.code, profile: t.code === "Extension" ? t.profile?.[0] : undefined }),
        ).join(" ");

        const binding = e.binding as { strength?: string; valueSet?: string; description?: string } | undefined;
        const bindingHtml = binding
            ? `${ctx.fns.core.tagBindingStrength(ctx, { s: binding.strength ?? "" })} ${ctx.fns.core.linkCanonical(ctx, { url: binding.valueSet, short: true })}${binding.description ? `<div class="mt-0.5 text-xs text-slate-500">${esc(binding.description)}</div>` : ""}`
            : "";

        const cons = (e.constraint as Array<{ key?: string; human?: string }> | undefined) ?? [];
        const consHtml = cons.length
            ? cons.map(c => `<div><code class="text-xs text-slate-900">${esc(c.key ?? "")}</code>: ${ctx.fns.md.mdInline(ctx, { md: c.human })}</div>`).join("")
            : "";

        return `<div${idAttr} class="scroll-mt-20 border-b border-slate-100 px-3 py-2">
            <div class="flex items-center gap-2 text-sm">
                ${ctx.fns.profile.flagsCell(ctx, { e })}
                <code class="font-semibold text-slate-900">${heading}</code>
            </div>
            <dl class="mt-1 text-xs">
                ${field("Definition", ctx.fns.md.mdInline(ctx, { md: (e.short as string | undefined) ?? (e.definition as string | undefined) }))}
                ${field("Comment", ctx.fns.md.mdInline(ctx, { md: e.comment as string | undefined }))}
