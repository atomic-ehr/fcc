...
export default function $section_formalViews(ctx: Context, opts: { resource: types.fcc.Resource }): { title: string; id: string; html: string } | null {
    ...
    const diffPanel  = ctx.fns.profile.elementTable(ctx, { elements, defnHref: h.definitions });
    const keyPanel   = keyEls.length ? ctx.fns.profile.elementTable(ctx, { elements: keyEls, defnHref: h.definitions }) : "";
    const bindPanel  = ctx.fns.profile.bindingsTable(ctx, { elements });
    const constPanel = ctx.fns.profile.constraintsTable(ctx, { elements });
    ...
    const innerTabs = ctx.fns.profile.profileTabs(ctx, { tabs: panels.map(p => ({ key: p.key, label: p.label, anchor: ANCHOR[p.key] })) });
    const innerHtml = panels.map(p =>
        `<div id="${ANCHOR[p.key]}" data-show="$sdtab === '${p.key}'"${p.key === activeInner ? "" : ` style="display:none"`}>${p.html}</div>`,
    ).join("");

    const baseLink = ctx.fns.core.linkCanonical(ctx, { url: d.baseDefinition as string });
    const html = `
        <p class="mt-1 text-xs text-slate-500">This structure is derived from ${baseLink}.</p>
        <div data-signals="{sdtab: '${activeInner}'}">
            ${innerTabs}
            ${innerHtml}
            ${ctx.fns.profile.flagLegend(ctx)}
        </div>
        <script>${ctx.fns.profile.tabHashScript(ctx)}</script>`;
