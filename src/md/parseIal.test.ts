...
test("parseIal: class marker", () => {
    const c = mk();
    expect(c.fns.md.parseIal(c, { raw: "{:.stu-note}" })).toEqual({ classes: ["stu-note"], id: undefined, directive: false });
});
test("parseIal: class + id", () => {
    const c = mk();
    expect(c.fns.md.parseIal(c, { raw: "{:.no_toc #translations}" })).toEqual({ classes: ["no_toc"], id: "translations", directive: false });
});
test("parseIal: bare id is not a directive", () => {
    const c = mk();
    expect(c.fns.md.parseIal(c, { raw: "{:#anchor}" })).toEqual({ classes: [], id: "anchor", directive: false });
});
test("parseIal: {::options} and {:toc} are directives", () => {
    const c = mk();
    expect(c.fns.md.parseIal(c, { raw: "{::options x}" }).directive).toBe(true);
    expect(c.fns.md.parseIal(c, { raw: "{:toc}" }).directive).toBe(true);
});
