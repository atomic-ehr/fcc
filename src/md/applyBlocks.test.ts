...
const mk = (opts: Record<string, unknown> = {}) => { const c: any = { state: {}, fns: {}, bundle: { resources: new Map(), byCanonical: new Map() } }; loadAll(c); c.fns.core.enable(c, { opts }); return c; };

test("applyBlocks: trailing stu-note → callout, no leak", () => {
    const c = mk();
    const out = c.fns.md.applyBlocks(c, { html: "<p>A note {:.stu-note}</p>" });
...
test("applyBlocks: standalone marker attaches to preceding block", () => {
    const c = mk();
    const out = c.fns.md.applyBlocks(c, { html: "<p>Body text</p>\n<p>{:.stu-note}</p>" });
...
test("applyBlocks: REGRESSION — does not swallow a fenced code block before a marker", () => {
    ...
    const out = c.fns.md.applyBlocks(c, { html });
...
test("applyBlocks: grid is transparent (class attached, no box)", () => {
    const c = mk();
    const out = c.fns.md.applyBlocks(c, { html: "<table><tr><td>x</td></tr></table>\n<p>{:.grid}</p>" });
...
test("applyBlocks: unknown class strips marker, keeps content", () => {
    const c = mk();
    const out = c.fns.md.applyBlocks(c, { html: "<p>Hello {:.totally-unknown}</p>" });
...
test("applyBlocks: dropBalloterNotes removes note-to-balloters", () => {
    ...
    const out = c.fns.md.applyBlocks(c, { html: "<p>ballot only {:.note-to-balloters}</p>" });
