...
export default function renderCanonical(ctx: Context, opts: { resource: types.fcc.Resource; strip?: string }): string {
    ...
    const meta = ctx.fns.core.canonicalMeta(ctx, { resource: r });
    const strip = opts.strip ?? ctx.fns.core.canonicalTabStrip(ctx, { resource: r, activeId: "content" });

    let n = 0;
    const parts: string[] = [];
    for (const id of ctx.fns.core.sectionsFor(ctx, { resource: r })) {
        const fn = ctx.fns.core.resolveFn(ctx, { key: `$section_${id}` });
        if (typeof fn !== "function") continue;
        const s = fn(ctx, { resource: r }) as { title: string; id: string; html: string } | null;
        if (!s || !s.html) continue;
        parts.push(s.title
            ? `${ctx.fns.core.sectionHeader(ctx, { num: `1.${++n}`, title: s.title, id: s.id })}${s.html}`
            : s.html);
    }

    const body = `
        ${ctx.fns.core.pageHeader(ctx, { title: meta.title, kind: meta.kind, d })}
        ${strip}
        ${ctx.fns.core.urlVersionStrip(ctx, { d })}
        ${parts.join("\n")}
    `;
    return ctx.fns.core.layout(ctx, { title: meta.title, content: body, breadcrumb: meta.breadcrumb, activeNav: meta.activeNav });
}
