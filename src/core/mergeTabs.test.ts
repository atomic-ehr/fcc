...
test("mergeTabs: remove + extend over default", () => {
    ...
    const merged = c.fns.core.mergeTabs(c, { defaults, overrides: { A: { remove: ["b"], extend: [{ id: "c", label: "C", kind: "companion", render: "z", suffix: ".c" }] } } });
    expect(merged.A.map((t: any) => t.id)).toEqual(["a", "c"]);
});
test("mergeTabs: array override replaces the set", () => {
    const c = mk();
    const merged = c.fns.core.mergeTabs(c, { defaults: { A: [{ id: "a" }] }, overrides: { A: [{ id: "z", label: "Z", kind: "main", render: "r", suffix: "" }] } });
