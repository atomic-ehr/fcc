export default function layout(
    ctx: Context,
    opts: { title: string; content: string; breadcrumb?: types.core.Breadcrumb; activeNav?: string },
): string {
...
${ctx.fns.core.topBar(ctx, { active: activeNav })}
${ctx.fns.core.buildInfoBanner(ctx)}

<div class="mx-auto flex max-w-screen-2xl" data-signals="{nav: true}" data-persist="nav">
    ${ctx.fns.core.sidebar(ctx)}
    <main class="min-w-0 flex-1 bg-white px-6 py-6 lg:px-10">
        <div class="mb-3 flex items-center gap-2">
            ...
            ${ctx.fns.core.renderBreadcrumb(ctx, { crumbs: breadcrumb })}
        </div>
        ${opts.content}
        <footer class="mt-16 border-t border-slate-200 pt-4 text-xs text-slate-500">
            ${ctx.fns.core.featureOn(ctx, { name: "igFooter" }) ? `<p>This page is part of the <span class="font-medium text-slate-700">${esc(ctx.cfg.title ?? ctx.cfg.id)}</span> (v${esc(ctx.cfg.version)}) based on FHIR ${esc(ctx.target.fhir)} — package <code class="rounded bg-slate-100 px-1">${esc(ctx.cfg.id)}#${esc(ctx.cfg.version)}</code>.</p>` : ""}
