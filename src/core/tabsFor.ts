...
export default function tabsFor(
...
): Array<{ d: types.core.TabDescriptor; href: string; rawName?: string }> {
    const r = opts.resource;
    const reg = (ctx.state.site?.tabRegistry ?? {}) as Record<string, types.core.TabDescriptor[]>;
    ...
    const out: Array<{ d: types.core.TabDescriptor; href: string; rawName?: string }> = [];
    for (const d of set) {
        if (d.avail) {
            const pred = ctx.fns.core.resolveFn(ctx, { key: d.avail });
