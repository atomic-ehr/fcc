...
export default function enable(ctx: Context, opts: { opts?: types.core.SiteOpts } = {}): void {
    const o = opts.opts ?? {};

    const tabRegistry = ctx.fns.core.mergeTabs(ctx, {
        defaults: ctx.fns.core.tabDefaults(ctx),
        overrides: o.tabs,
    });

    // Per-resourceType Content section lists; a project key replaces the default.
    const sectionRegistry = { ...ctx.fns.core.sectionDefaults(ctx), ...(o.sections ?? {}) };

    // Blocks: defaults overlaid with author entries; `false` disables a built-in.
    const blockRegistry: Record<string, types.md.BlockDescriptor> = { ...ctx.fns.md.blockDefaults(ctx) };
