export default function artifactTable(
    ...
    const rows = sorted.map(r => {
        ...
        const id    = esc(ctx.fns.site_core.idOf(ctx, { resource: r }));
        ...
        return `
            <tr class="hover:bg-sky-50/40 align-top">
                ...
                <td class="px-3 py-2 text-sm text-slate-600">${ctx.fns.site_md.mdInline(ctx, { md: desc })}</td>
            </tr>`;
    }).join("");
    ...
    return `<section class="mt-8" id="${esc(opts.anchor)}">
        ...
        ${ctx.fns.site_core.dataTable(ctx, { columns: ["Name", "Id", "Description"], rows })}
    </section>`;
}
