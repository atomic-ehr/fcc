...
export default function conceptTable(
    ...
    const rows = opts.concepts.map(c => {
        const sysCell = opts.showSystem ? `<td class="px-3 py-1.5 text-xs text-slate-500">${c.system ? ctx.fns.core.linkCanonical(ctx, { url: c.system, short: true }) : ""}</td>` : "";
        const defCell = opts.showDefinition ? `<td class="px-3 py-1.5 text-xs text-slate-600">${ctx.fns.md.mdInline(ctx, { md: c.definition })}</td>` : "";
