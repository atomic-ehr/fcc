...
export default function vsCld(ctx: Context, opts: { compose: Record<string, unknown> | undefined }): string {
    ...
    const block = (verb: "include" | "exclude", inc: Record<string, unknown>): string => {
        ...
        const sysLink = system ? ctx.fns.core.linkCanonical(ctx, { url: system, short: true }) : "";
        ...
        if (valueSets.length) {
            return `<div class="mt-3 text-sm text-slate-700">Import all the codes that are contained in
                ${valueSets.map(v => ctx.fns.core.linkCanonical(ctx, { url: v, short: true })).join(", ")}</div>`;
        }
        if (concepts.length) {
            const enriched = concepts.map(c => ({ ...c, system, display: c.display ?? ctx.fns.terminology.displayFor(ctx, { system, code: c.code }) }));
            return `<div class="mt-3 text-sm text-slate-700">${verbText} these codes as defined in ${sysLink || "the system"}:</div>
                <div class="mt-1">${ctx.fns.terminology.conceptTable(ctx, { concepts: enriched })}</div>`;
        }
        if (filters.length) {
