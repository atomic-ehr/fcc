...
export default function generateNarrative(ctx: Context, opts: { resource: types.fcc.Resource }): string {
    ...
    if (opts.resource.resourceType === "Bundle") return ctx.fns.narrative.bundleNarrative(ctx, { resource: opts.resource });
    ...
    const rows = Object.entries(d)
        .filter(([k]) => !skip.has(k))
        .map(([k, val]) => {
            const rendered = ctx.fns.narrative.fhirValue(ctx, { value: val, depth: 1 });
            if (!rendered) return "";
