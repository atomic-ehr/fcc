export default function renderArtifacts(ctx: Context, _opts: {} = {}): string {
    ...
    const split = ctx.fns.core.featureOn(ctx, { name: "splitExtensions" });
    ...
    const sortedTypes = Object.entries(groups)
        .sort(([a], [b]) => ctx.fns.core.order(ctx, { t: a }) - ctx.fns.core.order(ctx, { t: b }) || a.localeCompare(b))
        .map(([type, list]) => ({ type, list, label: ctx.fns.core.humanType(ctx, { t: type }) }));

    const conformanceBlocks = sortedTypes
        .map(g => ctx.fns.artifacts.artifactTable(ctx, { label: g.label, anchor: g.type, list: g.list }))
        .join("");

    const examplesBlock = examples.length
        ? ctx.fns.artifacts.artifactTable(ctx, { label: "Examples", anchor: "examples", list: examples })
        : "";
    ...
    return ctx.fns.core.layout(ctx, {
        ...
        content: `
            ...
            ${ctx.fns.artifacts.pageToc(ctx, { items: tocItems })}
            ${conformanceBlocks}
            ${examplesBlock}
        `,
