...
export default function fhirValue(ctx: Context, opts: { value: unknown; depth?: number }): string {
    ...
    if (Array.isArray(v)) {
        const parts = v.map(x => ctx.fns.site_narrative.fhirValue(ctx, { value: x, depth })).filter(Boolean);
    ...
    if (typeof o.url === "string" && depth > 0) {
        ...
        const inner = valKey ? ctx.fns.site_narrative.fhirValue(ctx, { value: o[valKey], depth: depth + 1 })
            : (Array.isArray(o.extension) ? ctx.fns.site_narrative.fhirValue(ctx, { value: o.extension, depth: depth + 1 }) : "");
    ...
    const pairs = Object.entries(o)
        ...
        .map(([k, val]) => {
            const rv = ctx.fns.site_narrative.fhirValue(ctx, { value: val, depth: depth + 1 });
