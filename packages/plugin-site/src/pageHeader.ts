export default function pageHeader(ctx: Context, opts: { title: string; kind: string; d: Record<string, unknown> }): string {
    const safeTitle = ctx.fns.site.htmlEscape(ctx, { s: opts.title });
    const safeKind  = ctx.fns.site.htmlEscape(ctx, { s: opts.kind });
    const safeId    = ctx.fns.site.htmlEscape(ctx, { s: (opts.d.id as string) ?? "" });
    const statusPill = opts.d.status
        ? `<span class="rounded-full bg-emerald-100 px-2 py-0.5 text-[11px] font-semibold uppercase tracking-wide text-emerald-800">${ctx.fns.site.htmlEscape(ctx, { s: opts.d.status as string })}</span>`
        : "";
    return `
