...
export default function detailTable(ctx: Context, opts: { elements: Array<Record<string, unknown>>; anchors?: boolean }): string {
    ...
    const blocks = opts.elements.map(e => {
        ...
        const bindingHtml = binding
            ? `${ctx.fns.site.tagBindingStrength(ctx, { s: binding.strength ?? "" })} ${ctx.fns.site.linkCanonical(ctx, { url: binding.valueSet, short: true })}${binding.description ? `<div class="mt-0.5 text-xs text-slate-500">${esc(binding.description)}</div>` : ""}`
            : "";
