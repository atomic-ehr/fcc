...
export default async function sdCompanionPages(
    ...
    return [
        { name: h.definitions, content: ctx.fns.site.renderDefinitionsPage(ctx, { resource: r }) },
        { name: h.mappings,    content: ctx.fns.site.renderMappingsPage(ctx, { resource: r }) },
        { name: h.examples,    content: ctx.fns.site.renderExamplesPage(ctx, { resource: r }) },
        { name: h.jsonPage,    content: await ctx.fns.site.renderProfileJsonPage(ctx, { resource: r }) },
