...
export default function applyBlocks(ctx: Context, opts: { html: string }): string {
    ...
    const reg = (ctx.state.site?.blockRegistry ?? {}) as Record<string, types.site.BlockDescriptor>;
    ...
    h = h.replace(
        ...
        (_m, tag, attrs, inner, raw) => {
            const ial = ctx.fns.site.parseIal(ctx, { raw });
            ...
            const d = reg[cls]!;
            if (d.render) {
                const fn = (ctx.fns.site as any)[d.render];
