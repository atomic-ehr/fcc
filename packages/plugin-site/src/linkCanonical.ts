...
export default function linkCanonical(ctx: Context, opts: { url: string | undefined; short?: boolean }): string {
    ...
    if (opts.short) {
        const label = target ? ctx.fns.site.shortLabel(ctx, { resource: target }) : (bare.split("/").pop() || bare);
        const tip = ` title="${esc(url)}"`;
        return target
            ? `<a class="text-sky-700 hover:underline"${tip} href="${ctx.fns.site.pageHref(ctx, { resource: target })}">${esc(label)}</a>`
            : `<span${tip} class="text-slate-600">${esc(label)}</span>`;
    }

    const safe = esc(url);
    return target
        ? `<a class="text-sky-700 hover:underline" href="${ctx.fns.site.pageHref(ctx, { resource: target })}"><code class="text-xs">${safe}</code></a>`
