...
export default async function renderProfileJsonPage(ctx: Context, opts: { resource: types.fcc.Resource; strip?: string }): Promise<string> {
    ...
    const body = `
        ...
        ${opts.strip ?? ctx.fns.site.formatChips(ctx, { resource: r, active: "json" })}
