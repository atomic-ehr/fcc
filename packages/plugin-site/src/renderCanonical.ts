...
export default function renderCanonical(ctx: Context, opts: { resource: types.fcc.Resource; strip?: string }): string {
    ...
    const meta = ctx.fns.site.canonicalMeta(ctx, { resource: r });
    const strip = opts.strip ?? ctx.fns.site.canonicalTabStrip(ctx, { resource: r, activeId: "content" });

    let n = 0;
    const parts: string[] = [];
    for (const id of ctx.fns.site.sectionsFor(ctx, { resource: r })) {
        const fn = (ctx.fns.site as any)[`$section_${id}`];
        if (typeof fn !== "function") continue;
        const s = fn(ctx, { resource: r }) as { title: string; id: string; html: string } | null;
        if (!s || !s.html) continue;
        parts.push(s.title
            ? `${ctx.fns.site.sectionHeader(ctx, { num: `1.${++n}`, title: s.title, id: s.id })}${s.html}`
            : s.html);
    }

    const body = `
        ${ctx.fns.site.pageHeader(ctx, { title: meta.title, kind: meta.kind, d })}
        ${strip}
        ${ctx.fns.site.urlVersionStrip(ctx, { d })}
        ${parts.join("\n")}
    `;
    return ctx.fns.site.layout(ctx, { title: meta.title, content: body, breadcrumb: meta.breadcrumb, activeNav: meta.activeNav });
}
