...
export default function elementTable(ctx: Context, opts: { elements: Array<Record<string, unknown>> }): string {
    const els = opts.elements;
    if (!els.length) {
        return `<p class="px-3 py-4 text-sm text-slate-500">No elements.</p>`;
    }
    const rows = els.map((e, i) => ctx.fns.site.elementRow(ctx, { e, isLast: i === els.length - 1 })).join("");
